...
class TAAccelerator final : public Accelerator {
private:
  static TAAccelerator *instance;
  TAAccelerator();

public:
  /// Singleton should not be clonable or assignable.
  TAAccelerator(TAAccelerator &) = delete;
  void operator=(const TAAccelerator &) = delete;

  ~TAAccelerator();

  /// Creates an instance on the first invocation. Subsequent invocations
  /// return the existing instance.
  static TAAccelerator *getInstance();

  /// Define classof to be able to use isa<>, cast<>, dyn_cast<>, etc.
  static bool classof(const Accelerator *accel) {
    return accel->getKind() == Accelerator::Kind::TA;
  }
  static bool classof(const TAAccelerator *) { return true; }
